// Agent task: works on a pull request, an issue or a chat branch, commits
// its changes, opens a pull request when needed and replies with actions.
...
export async function runAgent(job: Job, deps: RunDeps): Promise<ReviewRun> {
  ...
  await live.guard([run], () => live.track(run, async () => {
    ...
    await git(cwd, ["reset", "--hard", "--quiet"]).then(() => git(cwd, ["clean", "-fdq"]), () => "");
    ...
    const session = await deps.backend.open({ cwd, model: run.model, write: true, log: (line) => log(`[${AGENT}] ${line}`), tool: live.tool(run) });
    try {
      const reply = await ask(run, session, buildAgentPrompt(context), AGENT_RETRY_PROMPT, (text) => parseAgentReply(text, settings), log);
      ...
      const commit = changed ? await commitAndPush(cwd, { owner, repo, branch }, [title, `Asked ${job.chat ? "on the Shrike website" : `in #${pr?.number ?? issue!.number}`}.`], await deps.autofix!.identity(), deps.autofix!.remote) : null;
      const at = commit ?? sha;
