...
10. Every finished review, the capture, the Shriken run and the autofix run are reported back to the API so the website shows history and cost. One JSON report per run is also written to the `reports` output directory.
...
| Fix, Re-run or Ask on a `shrike/<review>` check | the Shrike GitHub App dispatches the autofix, that review, or an ask that explains its findings |
